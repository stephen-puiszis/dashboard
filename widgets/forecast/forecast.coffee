class Dashing.Forecast extends Dashing.Widget
  ...
  @accessor 'hour_icon', (hour) ->
    getIcon(@get("hourly[#{hour}]"))
  ...
  unpackHours: (hoursData) ->
    hours = []
    for hour in hoursData
      hourObj = {
        time: hour['time']
        summary: hour['summary']
        precipitation: "#{Math.round(hour['precipitation'] * 100)}%"
        temp: "#{hour['temp']}&deg;"
        icon: getIcon(@get(hour['icon']))
      }
      hours.push hourObj
    @set 'hours', hours
  ...
  unpackWeek: (thisWeek) ->
    ...
    for day in thisWeek
      dayObj = {
        time: day['time'],
        temp_range: "#{day['min_temp']}&deg; - #{day['max_temp']}&deg;",
